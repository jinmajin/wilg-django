...
from userena.forms import (SignupForm, SignupFormOnlyEmail, AuthenticationForm,
                           ChangeEmailForm, EditProfileForm, MajorForm,
                           ExecForm, AffiliationForm)
...
import warnings
from mit import ScriptsRemoteUserMiddleware

from django.forms.models import inlineformset_factory
from accounts.models import Major, MinorConc, Location, ExecRole, Affiliation
...
def signup(request, signup_form=SignupForm,
    ...
    **Context**
    ...
    form = signup_form()
    if ScriptsRemoteUserMiddleware.header in request.META.keys() and 'kerberos' in request.META.keys():
        form=signup_form(initial={'email':request.META[ScriptsRemoteUserMiddleware.header], 'username':request.META['kerberos']})
...
def signin(request, auth_form=AuthenticationForm,
    ...
    **Context**
    ...
    if request.method == 'POST':
        ...
        # they've been authenticated via certificates
        # but they may not have a profile yet!

        if request.user.is_active:
            request.user.backend = 'userena.backends.UserenaAuthenticationBackend'
            ...
            #send a signal that a user has signed in
            userena_signals.account_signin.send(sender=None, user=request.user)
            # Whereto now?
            redirect_to = redirect_signin_function(
...
def profile_edit(request, username, edit_profile_form=EditProfileForm,
    ...
    **Context**
    ...
    major_formset = inlineformset_factory(get_profile_model(), Major)
    minor_formset = inlineformset_factory(get_profile_model(), MinorConc)
    location_formset = inlineformset_factory(get_profile_model(), Location)
    exec_formset = inlineformset_factory(get_profile_model(), ExecRole)
    affiliation_formset = inlineformset_factory(get_profile_model(), Affiliation, form=AffiliationForm)

    if request.method == 'POST':
        form = edit_profile_form(request.POST, request.FILES, instance=profile,
                                 initial=user_initial)
        majorset = major_formset(request.POST, request.FILES, instance=profile)
        minorset = minor_formset(request.POST, request.FILES, instance=profile)
        locationset = location_formset(request.POST, request.FILES, instance=profile)
        execset = exec_formset(request.POST, request.FILES, instance=profile)
        affiliationset = affiliation_formset(request.POST, request.FILES, instance=profile)

        if form.is_valid():
            profile = form.save()

            if majorset.is_valid():
                majorset.save()
            if minorset.is_valid():
                minorset.save()
            if locationset.is_valid():
                locationset.save()
            if execset.is_valid():
                execset.save()
            if affiliationset.is_valid():
                affiliationset.save()
    ...
    if not extra_context: extra_context = dict()
    extra_context['form'] = form
    extra_context['major_formset'] = major_formset(instance=profile)
    extra_context['minor_formset'] = minor_formset(instance=profile)
    extra_context['exec_formset'] = exec_formset(instance=profile)
    extra_context['location_formset'] = location_formset(instance=profile)
    extra_context['affiliation_formset'] = affiliation_formset(instance=profile)
